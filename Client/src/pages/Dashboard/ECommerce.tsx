...
const appContext = useContext(AppContext);
  ...
  useEffect(() => {
    const fetchTotalConnects = async () => {
      try {
        ...
        setTotalConnects(response.data.count); // Assuming the API returns { totalConnects: number }
      } catch (error) {
  ...
  // Logic for determining levelUp and levelDown based on connects
  const levelUp = totalConnects !== null && totalConnects > 500;  // Example: levelUp if totalConnects > 500
  const levelDown = totalConnects !== null && totalConnects <= 500;  // Example: levelDown if totalConnects <= 500
  ...
  return (
    <>
      ...
      <CardDataStats
          ...
          rate="1.2%"  // Example rate
          levelUp={levelUp}  // Condition for levelUp
          levelDown={levelDown}  // Condition for levelDown
        >
          {/* SVG Icon for connects */}
          <svg
            width="20"
            height="20"
            viewBox="0 0 24 24"
        ...
        <CardDataStats title="Total Profit" total="Ksh 45,2K" rate="4.35%" levelUp>
          <svg
